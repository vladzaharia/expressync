...
```
manage.example.com   A    <traefik-ipv4>
example.com          A    <traefik-ipv4>
mail.example.com     A    <traefik-ipv4>   (or Cloudflare-routed)
assets.example.com   CNAME <r2-bucket>.r2.cloudflarestorage.com
```
...
### 2. SPF + DKIM + DMARC for `example.com`
...
CF_EMAIL_WORKER_URL=https://mail.example.com
CF_EMAIL_WORKER_SECRET=<same-secret-as-POLARIS_SECRET_A>
EMAIL_FROM=Polaris Express <noreply@example.com>
EMAIL_FROM_ADMIN=ExpresSync Operator <admin-noreply@example.com>
...
OPERATOR_CONTACT_EMAIL=support@example.com
```
...
- `polaris-admin` router (manage.example.com)
- `polaris-customer` router (example.com)
...
curl -I https://example.com/sessions
# 302 → /login (no session)

# Admin surface
curl -I https://manage.example.com/login
# 200 + ExpresSync admin login

# Legacy redirect
curl -I https://expressync.polaris.gdn/sync
# 301 → https://manage.example.com/sync
...
- Send yourself a test magic link from `example.com/login`
- Verify the email arrives within 30s, lands in inbox (not spam)
- Click the link → verify session is created on `example.com`
...
- **DMARC escalation timeline** — start at `p=none` for 30 days, then
  ...
  `dmarc-reports@example.com` mailbox.
