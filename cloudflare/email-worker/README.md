...
Email Service. Deployed at `https://mail.example.com`.
...

{
  ...
  "html": "<!doctype html>…",
  "text": "Sign in: https://example.com/auth/verify?token=…",
  "category": "magic-link",
  "from": "Polaris Express <noreply@example.com>",
...
5. **Sender allowlist** — `from:` address must be one of:
   * `noreply@example.com` (customer-facing emails, Polaris brand)
   * `admin-noreply@example.com` (admin-facing, ExpresSync brand)
...
6. Once live, point a Worker route at `mail.example.com/*` from the
...
the `example.com` zone, the `send_email` bindings in `wrangler.jsonc`
...
(`assets.example.com`, served from R2). These must be uploaded
**before the first send** or images will appear broken in customer
inboxes:

* `https://assets.example.com/email/polaris-logo-160.png` — 160 × 40
* `https://assets.example.com/email/polaris-logo-320.png` — 320 × 80 (2x)
* `https://assets.example.com/email/expressync-logo-160.png` — 160 × 40
* `https://assets.example.com/email/expressync-logo-320.png` — 320 × 80 (2x)
