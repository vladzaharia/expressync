...
async function detectStream(chargeBoxId: string, pairingCode: string) {
  const url = `${BASE_URL}/api/auth/scan-detect?chargeBoxId=${
...
async function clearVerifications() {
  await pgQuery(
    ctx,
    null,
    `DELETE FROM verifications WHERE identifier LIKE 'scan-pair:%';`,
  );
}

async function clearOperationLog() {
  await pgQuery(ctx, null, `DELETE FROM charger_operation_log;`).catch(
    () => {/* table optional */},
  );
}
...
Deno.test({
  ...
  async fn(t) {
    ...
    // --- 1. Control ---------------------------------------------------------
    await t.step(
      "1. control: TAG_GOOD without armed intent → Accepted, tx starts",
      async () => {
        ...
        try {
          ...
          assertEquals(
            auth.status,
            "Accepted",
            "control authorize should be Accepted",
          );
          ...
          await assertEventually(async () => {
            const rows = await mysqlQuery(
              ctx,
              STEVE_DB_PASSWORD,
              `SELECT transaction_pk FROM transaction WHERE transaction_pk = ${tx.transactionId};`,
            );
    ...
    // --- 2. Happy path login ------------------------------------------------
    await t.step(
      "2. happy path: armed → Authorize Blocked, scan-detect SSE fires, scan-login 200",
      async () => {
        await clearVerifications();
        ...
        try {
          ...
          assertEquals(
            auth.status,
            "Blocked",
            "preauth must override to Blocked",
          );
          ...
          assertEquals(
            loginResp.status,
            200,
            `scan-login expected 200, got ${loginResp.status} ${await loginResp
              .clone().text()}`,
          );
    ...
    // --- 3. Wrong charger ---------------------------------------------------
    await t.step(
      "3. wrong charger: arm CB_A, scan at CB_B → Accepted, tx starts, no SSE",
      async () => {
        await clearVerifications();
        const { pairingCode } = await armScanPair(CB_A);
        const sse = await detectStream(CB_A, pairingCode);
        ...
        try {
          ...
          assertEquals(
            sseFired,
            false,
            "SSE must NOT fire for the wrong charger",
          );
          await sim.stopTransaction(tx.transactionId);
        } finally {
          sse.close();
    ...
    await t.step(
      "4. unknown tag: original INVALID stays INVALID; scan.intercepted still fires",
      async () => {
        await clearVerifications();
        const { pairingCode } = await armScanPair(CB_A);
        const sse = await detectStream(CB_A, pairingCode);
        ...
        try {
          ...
          assertEquals(
            auth.status,
            "Invalid",
            "unknown tag stays Invalid (preauth only overrides ACCEPTED)",
          );
          const msg = await sse.next(10_000);
          ...
          assertEquals(
            payload.idTag,
            TAG_UNKNOWN,
            "scan.intercepted forwards even for unknown tags",
          );
        } finally {
          sse.close();
          await sim.dispose();
    ...
    await t.step({
      name:
        "5. [slow] hook timeout: SteVe fails open, Authorize Accepted, tx starts",
      ignore: !RUN_SLOW,
      fn: async () => {
        await clearVerifications();
        ...
        try {
          ...
          await assertEventually(async () => {
            ...
            timeoutMs: 180_000,
            intervalMs: 2_000,
            message: "steve not back after restart",
          });
    ...
    await t.step({
      name: "6. [slow] hook 5xx: fail open",
      ignore: !RUN_SLOW,
      fn: async () => {
        await clearVerifications();
        ...
        const stubUrl =
          `http://host.docker.internal:${port}/api/ocpp/pre-authorize`;
    ...
    // --- 7. Malformed JSON --------------------------------------------------
    await t.step({
      name: "7. [slow] hook malformed JSON: fail open",
      ignore: !RUN_SLOW,
      fn: async () => {
        await clearVerifications();
        const stub = Deno.serve(
    ...
    // --- 8. HMAC mismatch ---------------------------------------------------
    await t.step(
      "8. HMAC mismatch: ExpresSync 401, SteVe fails open",
      async () => {
        await clearVerifications();
        // Override only ExpresSync's key so it diverges from SteVe's.
        Deno.env.set(
          "EXPRESSYNC_PREAUTH_HMAC_KEY",
          "DIFFERENT_KEY_" + crypto.randomUUID(),
        );
        try {
          ...
          try {
            ...
            assertEquals(
              sseFired,
              false,
              "no scan.intercepted on HMAC failure",
            );
          } finally {
            sse.close();
            await sim.dispose();
    ...
    await t.step({
      name:
        "9. [slow] watchdog: hook returns null but DB row armed → RemoteStop after StartTransaction",
      ignore: !RUN_SLOW,
      fn: async () => {
        await clearVerifications();
        ...
        const stub = Deno.serve(
          { port: 0, hostname: "0.0.0.0" },
          () =>
            new Response(JSON.stringify({ override: null }), {
              status: 200,
              headers: { "Content-Type": "application/json" },
            }),
        );
        ...
        try {
          ...
          try {
            ...
            await assertEventually(async () => {
              ...
              timeoutMs: 10_000,
              message: "watchdog never sent RemoteStop",
            });

            // Verify charger_operation_log row.
            await assertEventually(async () => {
              const rows = await pgQueryJson<{ params: { reason?: string } }>(
                ctx,
              ...
              return rows.length > 0 &&
                rows[0].params?.reason === "intercepted-for-login";
    ...
    // --- 10. Intent expired -------------------------------------------------
    await t.step(
      "10. intent expired: preauth finds no armed row → Accepted",
      async () => {
        await clearVerifications();
        const { pairingCode } = await armScanPair(CB_A);
    ...
    // blocked" copy.
    await t.step(
      "11. blocked tag: stays Blocked but scan.intercepted still fires",
      async () => {
        await clearVerifications();
        const { pairingCode } = await armScanPair(CB_A);
        const sse = await detectStream(CB_A, pairingCode);
        ...
        try {
          ...
          assertEquals(
            auth.status,
            "Blocked",
            "blocked tag stays Blocked from SteVe (override won't upgrade non-ACCEPTED)",
          );
          const msg = await sse.next(10_000);
          const payload = JSON.parse(msg.data) as { idTag: string };
          assertEquals(
            payload.idTag,
            TAG_BLOCKED,
            "scan.intercepted forwards even for blocked tags",
          );
        } finally {
          sse.close();
          await sim.dispose();
    ...
    await t.step(
      "12. concurrent intents: CB_A and CB_B intercepted independently",
      async () => {
        await clearVerifications();
    ...
    await t.step(
      "13. replay: two Authorize for same TAG → both Blocked, only one event",
      async () => {
        await clearVerifications();
        const { pairingCode } = await armScanPair(CB_A);
        const sse = await detectStream(CB_A, pairingCode);
        ...
        try {
          ...
          assertEquals(
            secondFired,
            false,
            "replay must not republish scan.intercepted",
          );
        } finally {
          sse.close();
          await sim.dispose();
    ...
    await t.step(
      "14. latency: 200 pre-authorize calls, p99 < 50ms",
      async () => {
        await clearVerifications();
        ...
        console.log(
          `[latency] p50=${p50.toFixed(2)}ms p99=${
            p99.toFixed(2)
          }ms n=${samples.length}`,
        );
