...
import { generatePublicId } from "@/src/lib/utils/public-id.ts";
...
export const users = pgTable("users", {
  ...
  // 8-char Crockford-ish public ID, printed on the user's QR card and
  // embedded in `https://example.com/user/<publicId>` for iOS-only
  // sign-in. Backfilled by migration 0046; auto-assigned via the
  // BetterAuth user-create hook for new rows.
  ...
  uniqueIndex("users_public_id_key").on(table.publicId),
  check(
    "users_role_check",
...
export const userMappings = pgTable("user_mappings", {
  ...
  // Per-device OCPP tag. NULL = customer's user-level meta-tag (used by
  // admins via the picker). Non-null = device-scoped tag auto-minted at
  // customer device registration; deregistering the device cascades the
  // delete. Migration 0046.
  ...
  index("idx_user_mappings_device_id").on(table.deviceId),
  check(
...
export const chargersCache = pgTable("chargers_cache", {
  ...
  // public landing URL `https://example.com/c/<publicId>` and
  // displayed in the top-right of every charger detail surface.
  // Backfilled by migration 0046; auto-assigned via Drizzle default
  // for new rows.
  publicId: text("public_id")
    .notNull()
    .$defaultFn(() => generatePublicId()),
  ...
  /// Structured address — populated via the inline editor on the
  /// charger detail page. Powers iOS Navigate and the future
  /// nearest-charger UX. All fields nullable; admins fill what they
  /// have. Migration 0046.
  ...
  /// ISO 3166-1 alpha-2 (e.g. "US", "GB"). Format-pinned by CHECK.
  addressCountry: text("address_country"),
  /// Decimal degrees. CHECK pins them to valid ranges.
  latitude: numeric("latitude", { precision: 9, scale: 6 }),
  longitude: numeric("longitude", { precision: 9, scale: 6 }),
  /// Soft-delete for unmanaged chargers. NULL = active. Listings hide
  /// rows where this is non-null; the public landing renders a
  /// "retired" copy. Migration 0046.
