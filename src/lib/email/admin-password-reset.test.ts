...
const URL = "https://manage.example.com/reset-password?token=abc123";
...
Deno.test("admin-password-reset: CTA URL is on manage.example.com", () => {
  const spec = buildAdminPasswordResetEmail({ to: "a@b", url: URL });
  assert(spec.cta);
  assertStringIncludes(spec.cta.url, "manage.example.com");
});
