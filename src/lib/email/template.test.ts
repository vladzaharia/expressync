...
Deno.test("renderTemplate produces valid HTML5 with DOCTYPE", async () => {
  ...
  assertEquals(out.fromHeader, "Polaris Express <noreply@example.com>");
  assertEquals(out.replyTo, "support@example.com");
});
...
Deno.test("CTA with http:// (insecure) throws", async () => {
  await assertRejects(
    () =>
      renderTemplate(
        baseSpec({
          cta: { label: "Plain", url: "http://example.com/sign-in" },
        }),
      ),
    Error,
    "not in the allowlist",
...
Deno.test("CTA with https://manage.example.com is allowed", async () => {
  const out = await renderTemplate(
    baseSpec({
      cta: {
        label: "Reset",
        url: "https://manage.example.com/reset-password?token=xyz",
      },
    }),
  );
  assertStringIncludes(out.html, "manage.example.com/reset-password");
});

Deno.test("CTA with mailto: is allowed", async () => {
  const out = await renderTemplate(
    baseSpec({
      cta: { label: "Email us", url: "mailto:support@example.com" },
    }),
  );
  assertStringIncludes(out.html, "mailto:support@example.com");
});
