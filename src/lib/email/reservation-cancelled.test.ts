...
Deno.test("reservation-cancelled: CTA url is on example.com", () => {
  const spec = buildReservationCancelledEmail({ to: "a@b", reservation });
  assert(spec.cta);
  assertEquals(spec.cta.url, "https://example.com/reservations/new");
...
Deno.test("reservation-cancelled: renders without throwing", async () => {
  ...
  assertStringIncludes(
    rendered.text,
    "https://example.com/reservations/new",
  );
});
