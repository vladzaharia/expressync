...
export default function UsersTable({
  ...
  // Accessible delete-confirm replacing the browser `confirm()`.
  const deleteTarget = useSignal<UserData | null>(null);
  ...
  const handleDelete = (userId: string) => {
    ...
    if (!user) return;
    // Open the confirm dialog; the actual DELETE runs from `confirmDelete`.
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        userList.value = userList.value.filter((u) => u.id !== target.id);
